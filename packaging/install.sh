...
log "  1. Nothing to approve for the zip itself -- import is not"
log "     signature-gated. Answer iTerm2's prompt for whether to launch"
log "     the script now (there's a \"Launch\" button in that dialog) or"
log "     leave it for later."
log "  2. If you didn't launch it from that dialog, start it yourself:"
log "     Scripts > GitPanel. (Verified 2026-09-18: iTerm2 imports this"
log "     script outside AutoLaunch/, so it does NOT start on its own"
log "     when iTerm2 quits and reopens -- see docs/ITERM2-FINDINGS.md.)"
log "  3. Show the toolbelt: View > Show Toolbelt (Shift-Cmd-B). This is a"
log "     separate toggle from step 4 below -- ticking the tool alone"
log "     will not display it."
log "  4. Tick View > Toolbelt > Git."
log ""
log "To remove it later, run packaging/uninstall.sh. Note: it deletes the"
log "script's files but cannot untick Git from View > Toolbelt -- that"
log "registration lives in iTerm2's own preferences, not in our files, so"
log "you'll need to untick it there yourself."
